...
InitInstructions()
{
  ...
  // draw the help
...

    {
      ...
      x = 4;
      StrCopy(str,
"Maryo ha catturato Donkie Kung e lo ha messo in gabbia. La missione di Junior \
è salvare suo padre. Deve evitare granchi e gli uccelli per prendere le chiavi \
e liberare Donkie Kung dalla gabbia.");
      ptrStr = str;
      ...
      x = 4;
      StrCopy(str,
"Usando la penna o i tasti, muovi Donkie Kung Junior come mostrato nell'immagine. \
Per prendere la chiave salta e vai verso sinistra.");
      ptrStr = str;
      while (StrLen(ptrStr) != 0) {
        UInt8 count = FntWordWrap(ptrStr, rect.extent.x-x);
      ...
      x = 4;
      StrCopy(str,
"Il Gioco A è per i principianti, il Gioco B richiede più pratica e \
coordinazione per giocare.");
      ptrStr = str;
      while (StrLen(ptrStr) != 0) {
        UInt8 count = FntWordWrap(ptrStr, rect.extent.x-x);
