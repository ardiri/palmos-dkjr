...
InitInstructions()
{
  const RectangleType     rect  = {{0,0},{142,305}};
  ...
  // draw the help
...

    {
      ...
      x = 4;
      StrCopy(str,
"Als Donkie Kung jr. retten Sie Ihren Vater aus den Fängen des Großwildjärs \
Maryo. Dabei müssen Sie klettern und springen, wie ein Großer und so \
manches Abenteuer gegen arglistige Vögel und andere Wesen bestehen.");
      ptrStr = str;
      ...
      x = 4;
      StrCopy(str,
"Indem Sie den Stylus order die Tasten benutzen können Sie Donkie \
Kung Junior bewegen. Springen Sie nach dem Schlüssel und bewegen Sie \
sich gleichzeitig nach Links.");
      ptrStr = str;
      while (StrLen(ptrStr) != 0) {
        UInt8 count = FntWordWrap(ptrStr, rect.extent.x-x);
