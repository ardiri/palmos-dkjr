...
#ifdef LANG_it
    StrCopy(str, (CHECK_SIGNATURE(prefs))
            ? "- VERSIONE REGISTRATA -" : "* NON REGISTRATA *");
#endif
#ifdef LANG_sv
    StrCopy(str, (CHECK_SIGNATURE(prefs))
            ? "- REGISTRERAD VERSION -" : "* OREGISTRERAD VERSION *");
#endif
#ifdef LANG_en
    StrCopy(str, (CHECK_SIGNATURE(prefs))
            ...
            GameAdjustmentType *adjustType) 
{
  ...
  switch (adjustType->adjustMode) 
  {
    ...
    case gameScoreCode:
         {
           ...
           for (i=1; (i < MAX_IDLENGTH) && (key[i] != '\0'); i++) {
             result -= (
                        ((gbls->adjustGame[index] & 0xc3) << 8) | 
                         (gbls->adjustGame[index] & 0x3c)
                       );
    ...
    case gameHighScore:
         switch (prefs->game.dkjr.gameType) 
         {
           case GAME_A:
